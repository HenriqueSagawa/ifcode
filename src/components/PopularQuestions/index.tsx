    ...
    <section ref={sectionRef} className="py-24 bg-black relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative">
        ...
        <div ref={questionsRef} className="space-y-6 max-w-5xl mx-auto flex flex-col gap-2">
          {questions.map((question) => (
            <Link key={question.id} href={`/pergunta/${question.id}`}>
              <div className="question-card opacity-0 translate-y-10 transition-all duration-700 group bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-2xl p-6 hover:border-green-500/50 cursor-pointer hover:scale-[1.02]">
                {/* Header da pergunta */}
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-green-400 font-semibold">{question.author}</p>
                      <p className="text-gray-500 text-sm">{question.time}</p>
                    </div>
                  </div>
                ...
                {/* Título da pergunta */}
                <h3 className="text-xl font-bold text-white mb-4 group-hover:text-green-400 transition-colors duration-300 leading-relaxed">
                  {question.title}
                </h3>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {question.tags.map((tag, tagIndex) => (
                    <span key={tagIndex} className="bg-gray-800 text-gray-300 px-3 py-1 rounded-full text-xs font-medium hover:bg-green-500/20 hover:text-green-400 transition-colors duration-300">
                      {tag}
                    </span>
                  ))}
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-6">
                    ...
                    <div className="flex items-center gap-2 text-blue-400">
                      ...
                      <span className="text-gray-400 text-sm">respostas</span>
                    </div>
                  </div>
        ...
        <div ref={ctaRef} className="text-center mt-12 opacity-0 transition-all duration-700">
          <Link href="/perguntas">
            <button className="bg-gradient-to-r from-green-500 to-green-600 text-black px-8 py-4 rounded-xl font-bold hover:from-green-400 hover:to-green-500 transition-all duration-300 whitespace-nowrap cursor-pointer shadow-lg shadow-green-500/25 hover:shadow-green-500/40 hover:scale-105">
